...
and const function templates. These recursively visit nested `std::variant` choices and convert
the concrete node reference to its public trait base. The first implementation deliberately
leaves validity to template instantiation: it does not constrain the accessor to traits
